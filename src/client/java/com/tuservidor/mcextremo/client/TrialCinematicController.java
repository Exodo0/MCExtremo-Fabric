package com.tuservidor.mcextremo.client;
...
public final class TrialCinematicController {
    ...
    private record BossIntro(int entityId, Vec3d fallbackPos, int totalTicks, int ticksRemaining) {
        BossIntro tick() {
            return new BossIntro(entityId, fallbackPos, totalTicks, ticksRemaining - 1);
        }
    }
    ...
    public static void startBossIntro(int entityId, Vec3d fallbackPos, int durationTicks) {
        ...
        activeIntro = new BossIntro(entityId, fallbackPos, ticks, ticks);
    }
    ...
    public static Vec3d getEventCameraPos() {
        if (activeEventIntro == null) return Vec3d.ZERO;
        return activeEventIntro.center().add(0.0, 7.0, 0.0);
    }

    public static float getEventCameraYaw(float tickDelta) {
        if (activeEventIntro == null) return 0.0f;
        float elapsed = activeEventIntro.totalTicks() - activeEventIntro.ticksRemaining() + tickDelta;
        return elapsed * 0.85f;
    }

    public static float getEventCameraPitch() {
        return -82.0f;
    }
    ...
    private static void renderHud(DrawContext context, float tickDelta) {
        ...
        if (alpha > 0.25f) {
            ...
            context.drawCenteredTextWithShadow(client.textRenderer, Text.literal("El Coloso del Vacio desciende"), width / 2, height / 2 + 34, color);
        }
    }
