package com.tuservidor.mcextremo.command;
...
public class ModCommands {
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            var cmd = literal("mce_legacy_disabled");

            cmd.then(literal("vidas")
                .executes(ctx -> comandoVidas(ctx.getSource()))
                .then(argument("jugador", StringArgumentType.word())
                    .executes(ctx -> comandoVidasJugador(ctx.getSource(), StringArgumentType.getString(ctx, "jugador")))
                )
            );

            cmd.then(literal("setvidas")
            ...
            cmd.then(literal("pvp")
                .requires(s -> s.hasPermissionLevel(2))
                .then(argument("estado", StringArgumentType.word())
                    .executes(ctx -> comandoPvp(ctx.getSource(), StringArgumentType.getString(ctx, "estado")))
                )
            );

            cmd.then(literal("revivir")
                .requires(s -> s.hasPermissionLevel(2))
                .then(argument("jugador", StringArgumentType.word())
                    .executes(ctx -> comandoRevivir(ctx.getSource(), StringArgumentType.getString(ctx, "jugador")))
                )
            );

            cmd.then(literal("reload")
                .requires(s -> s.hasPermissionLevel(2))
                .executes(ctx -> comandoReload(ctx.getSource()))
            );

            cmd.then(literal("scoreboard")
                .requires(s -> s.hasPermissionLevel(2))
                .executes(ctx -> comandoScoreboard(ctx.getSource()))
            );

            cmd.then(literal("estado")
                .executes(ctx -> comandoEstado(ctx.getSource()))
                .then(argument("jugador", StringArgumentType.word())
                    .requires(s -> s.hasPermissionLevel(2))
                    .executes(ctx -> comandoEstadoJugador(ctx.getSource(), StringArgumentType.getString(ctx, "jugador")))
                )
            );

            cmd.then(literal("muertos")
                .requires(s -> s.hasPermissionLevel(2))
                .executes(ctx -> comandoMuertos(ctx.getSource()))
            );

            cmd.then(literal("mejorar")
                .executes(ctx -> comandoMejorar(ctx.getSource()))
            );

            cmd.then(literal("corazones")
                .executes(ctx -> comandoCorazones(ctx.getSource(), null))
                .then(argument("jugador", StringArgumentType.word())
                    .requires(s -> s.hasPermissionLevel(2))
                    .executes(ctx -> comandoCorazones(ctx.getSource(), StringArgumentType.getString(ctx, "jugador")))
                )
            );

            cmd.then(literal("revivearena")
                .requires(s -> s.hasPermissionLevel(2))
                .then(literal("set")
                    .executes(ctx -> comandoReviveArenaSet(ctx.getSource()))
                )
                .then(literal("test")
                    .executes(ctx -> comandoReviveArenaTest(ctx.getSource()))
                )
            );

            cmd.then(literal("trial")
                .executes(ctx -> comandoTrialVoluntario(ctx.getSource()))
                .then(literal("iniciar")
                    .requires(s -> s.hasPermissionLevel(2))
                    .then(argument("jugador", StringArgumentType.word())
                        .executes(ctx -> comandoTrialIniciar(ctx.getSource(), StringArgumentType.getString(ctx, "jugador")))
                    )
                )
            );

            cmd.then(literal("dias")
                .executes(ctx -> comandoDias(ctx.getSource()))
            );

            cmd.then(literal("saltardias")
                .requires(s -> s.hasPermissionLevel(2))
                .then(argument("cantidad", IntegerArgumentType.integer(1))
                    .executes(ctx -> comandoSaltarDias(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "cantidad")))
                )
            );

            cmd.then(literal("reducirdias")
                .requires(s -> s.hasPermissionLevel(2))
                .then(argument("cantidad", IntegerArgumentType.integer(1))
                    .executes(ctx -> comandoReducirDias(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "cantidad")))
                )
            );

            cmd.then(literal("spawnzombie")
                .requires(s -> s.hasPermissionLevel(2))
                .executes(ctx -> comandoSpawnZombie(ctx.getSource()))
            );

            cmd.then(literal("zombieinfo")
                .requires(s -> s.hasPermissionLevel(2))
                .executes(ctx -> comandoZombieInfo(ctx.getSource()))
            );

            cmd.then(literal("skills")
                .executes(ctx -> comandoSkills(ctx.getSource()))
                .then(literal("reset")
                    .requires(s -> s.hasPermissionLevel(2))
                    .then(argument("jugador", StringArgumentType.word())
                        .executes(ctx -> comandoSkillsReset(ctx.getSource(),
                            StringArgumentType.getString(ctx, "jugador")))
                    )
                )
                .then(literal("info")
            ...
            cmd.then(literal("config")
                ...
                .then(literal("get")
                    .then(argument("ruta", StringArgumentType.greedyString())
                        .executes(ctx -> comandoConfig(ctx.getSource(), "get", StringArgumentType.getString(ctx, "ruta")))
                    )
                )
                .then(literal("set")
                    .then(argument("ruta", StringArgumentType.word())
                        .then(argument("valor", StringArgumentType.greedyString())
                            .executes(ctx -> comandoConfigSet(ctx.getSource(),
                                StringArgumentType.getString(ctx, "ruta"),
                                StringArgumentType.getString(ctx, "valor")))
                        )
                    )
                )
                .then(literal("reload")
                    .executes(ctx -> comandoConfigReload(ctx.getSource()))
                )
                .then(literal("reset")
                    .executes(ctx -> comandoConfigReset(ctx.getSource()))
                )
            );

            cmd.executes(ctx -> comandoAyuda(ctx.getSource()));
    ...
    private static int comandoSkillsInfo(ServerCommandSource source) {
        ...
        source.sendFeedback(() -> Text.literal("\u00A7eDesbloqueadas: \u00A7f" + unlocked.size() + "/"
            + com.tuservidor.mcextremo.skilltree.Skill.values().length), false);
        source.sendFeedback(() -> Text.literal("\u00A7eNiveles: \u00A7f" + player.experienceLevel), false);
        for (com.tuservidor.mcextremo.skilltree.Skill skill : com.tuservidor.mcextremo.skilltree.Skill.values()) {
